...
r'''Simpleaudio doesn't seem to work.  It's designed to be given a long sound piece and
just play it the background while the program goes off and does other things...

Also doesn't work without pulseaudio.
'''
...
def init(channels=1, sample_rate=44100, block_duration=0.050, bits=16, callback=None):
    ...
    global Max_sound_level, Delta_t, Block_size, Block_times, Audio, Stream
    ...
    #Stream = Audio.open(format=Audio.get_format_from_width(bits//8),
    #                    channels=channels,
    #                    rate=sample_rate,
    #                    frames_per_buffer=Block_size,  # error with 2*Block_size
...
if __name__ == "__main__":
    ...
    if use_callback:
        ...
        for t in range(0, 2000, 50):
            ...
            start_time = time.perf_counter_ns()
            sa.play_buffer(samples, 1, 2, Sample_rate)
            now = time.perf_counter_ns()
            print("play_buffer delay", (now - start_time) / 1e6, "msec", t)
